...
import { helpTexto, estado } from './modules'
...
const VERBO_CANONICO: Record<string, string> = {}
for (const [canonico, sinonimos] of Object.entries(
  VERBOS
)) {
  for (const s of sinonimos) VERBO_CANONICO[s] = canonico
}
...
const ESPECIAIS: Record<string, () => string> = {
  ...
  comandos: () => helpTexto(),
  historico: () =>
    'Use "teo status" para info do sistema ou "teo listar chats" para ver chats.',
  history: () =>
    'Use "teo status" para info do sistema ou "teo listar chats" para ver chats.',
  log: () =>
    'Use "teo status" para info do sistema ou "teo listar chats" para ver chats.',
  status: () =>
    `TEO operacional  |  modo: ${estado.chatAtual ? 'ativo' : 'standby'}  |  chat: ${estado.chatAtual ?? 'nenhum'}`,
  info: () =>
    `TEO operacional  |  modo: ${estado.chatAtual ? 'ativo' : 'standby'}  |  chat: ${estado.chatAtual ?? 'nenhum'}`,
  estado: () =>
    `TEO operacional  |  modo: ${estado.chatAtual ? 'ativo' : 'standby'}  |  chat: ${estado.chatAtual ?? 'nenhum'}`,
  cancelar: () => {
    estado.esperandoNomeChat = false
    estado.esperandoNomeExclusao = false
    estado.esperandoNomeAbertura = false
    estado.esperandoNomeRenomear = false
    estado.esperandoNomeExportar = false
    estado.fuzzyPendente = null
  ...
  para: () =>
    'Use "teo ajuda" para ver os comandos disponíveis.',
  stop: () =>
    'Use "teo ajuda" para ver os comandos disponíveis.',
  sair: () => '__sair__',
  exit: () => '__sair__',
...
function melhorFuzzy(
  ...
  for (const opcao of lista) {
    ...
    if (
      Math.abs(opcao.length - palavra.length) >
...
const CHAT_DIR = path.resolve(
  __dirname,
  '../../src/memory/chatCache'
)

function sugerirChat(nomeDigitado: string): string | null {
...
function pareceMensagem(texto: string): boolean {
  ...
  if (palavras.length > 8) {
    const temPalavraNaoComando = PALAVRAS_NAO_COMANDO.some(
      (p) => t.includes(p)
    )
    if (temPalavraNaoComando) return true
  }

  // Começa com pergunta
  if (
    /^(como|o que|qual|por que|quando|onde|quem|me ajude|pode)\s/i.test(
      t
    )
  ) {
    return true
  ...
  if (/\w+(ando|endo|indo|ido|ê|eu)\s/.test(t)) return true
...
function pareceComandoParcial(texto: string): boolean {
  const t = texto.toLowerCase().trim()
  if (t.startsWith('teo ')) return true
  const palavras = t.split(/\s+/)
  if (palavras.length <= 4) {
    const primeiro = palavras[0] ?? ''
    if (primeiro && VERBO_CANONICO[primeiro]) return true
    const conhecidos = [
      'criar',
      'abrir',
      'fechar',
...
export function detectarComando(
  input: string
): ResultadoDetecao {
  ...
  const ehComandoParcial =
    pareceComandoParcial(textoOriginal)
  ...
  if (t.startsWith('teo ')) {
    ...
    const verboCorrigido = melhorFuzzy(
      primeiro,
      Object.values(VERBOS).flat()
    )
    if (!verboCorrigido) {
...
function processarVerbo(
  verbo: string,
  args: string,
  _verboOriginal?: string
): ResultadoDetecao {
  ...
  switch (verbo) {
    case 'criar': {
      const nomeChat =
        args.replace(/^(chat|novo)\s*/gi, '').trim() || ''
      if (nomeChat) {
        return {
          ehComando: true,
    ...
    case 'abrir': {
      ...
      const existe = fs.existsSync(
        path.join(chatDir, `${nomeChat}.json`)
      )
      if (existe) {
        return {
          ehComando: true,
    ...
    case 'fechar':
      return {
        ehComando: true,
        resultado: estado.chatAtual
          ? `Chat "${estado.chatAtual}" fechado.`
          : 'Nenhum chat aberto.',
        verboCorrigido: verbo,
      }
    ...
    case 'listar': {
      ...
      const lista = arquivos
        .map((f) => {
          ...
          return `  - ${nome}${nome === estado.chatAtual ? ' ◀ ativo' : ''}`
        })
        .join('\n')
      return {
        ehComando: true,
    ...
    case 'excluir': {
      ...
      for (const nome of nomes) {
        const existe = fs.existsSync(
          path.join(chatDir, `${nome}.json`)
        )
        if (existe) {
          fs.unlinkSync(path.join(chatDir, `${nome}.json`))
          if (estado.chatAtual === nome)
            estado.chatAtual = null
          excluidos.push(nome)
        } else {
      ...
      if (excluidos.length === 1)
        ...
        partes.push(
          `Chats excluídos: ${excluidos.map((n) => `"${n}"`).join(', ')}.`
        )
      if (naoEncontrados.length)
        partes.push(
          `Não encontrados: ${naoEncontrados.map((n) => `"${n}"`).join(', ')}.`
        )
      return {
        ehComando: true,
    ...
    case 'renomear': {
      ...
      if (!match || !match[1] || !match[2]) {
        return {
          ehComando: true,
          resultado:
            'Use: teo renomear chat <nome antigo> para <nome novo>',
          verboCorrigido: verbo,
        }
      }
      ...
      if (estado.chatAtual === oldName)
        estado.chatAtual = newName
      return {
        ehComando: true,
    ...
    case 'exportar': {
      ...
      for (const m of msgs)
        linhas.push(
          `[${m.role === 'user' ? 'Senhor' : 'TEO'}]: ${m.content}`,
          ''
        )
