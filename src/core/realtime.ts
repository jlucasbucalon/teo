...
const metricas: MetricasRealtime = {
  chamadas_totais: 0,
  sucessos: 0,
  falhas: 0,
...
export function obterMetricasRealtime(): MetricasRealtime {
  return { ...metricas }
}
...
function salvarNoCache<T>(chave: string, dados: T): void {
  cache.set(chave, { data: dados, timestamp: Date.now() })
}

export function limparCacheRealtime(): void {
  cache.clear()
}
...
async function obterHorarioWorldTimeAPI(
  timezone: string
): Promise<TimeInfo | null> {
  const cacheKey = `horario:${timezone}`
  const cached = obterDoCache<TimeInfo>(cacheKey)
  ...
  const data = await fazerRequisicaoComRetry<WtResponse>(
...
async function obterGeoIPAPI(): Promise<GeoInfo | null> {
  const cacheKey = 'geo:ip'
  const cached = obterDoCache<GeoInfo>(cacheKey)
  if (cached) return cached
  ...
  const data = await fazerRequisicaoComRetry<IpApiResponse>(
    'http://ip-api.com/json'
  )

  if (!data || data.status !== 'success') return null

  const resultado: GeoInfo = {
    ip: data.query,
    cidade: data.city,
...
function mapPaisParaMoeda(countryCode: string): string {
...
async function obterClimaOpenMeteo(
  cidade: string
): Promise<ClimaInfo | null> {
  const cacheKey = `clima:${cidade.toLowerCase()}`
  const cached = obterDoCache<ClimaInfo>(cacheKey)
  if (cached) return cached
  ...
  const geoData =
    await fazerRequisicaoComRetry<GeoResponse>(
  ...
  const weatherData =
    await fazerRequisicaoComRetry<WeatherResponse>(
      'https://api.open-meteo.com/v1/forecast',
      {
        params: {
          ...
          longitude: lon,
          current:
            'temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,wind_speed_10m,visibility,surface_pressure,uv_index',
          daily:
  ...
  const { condicao, icone } = codigoParaCondicao(
    current.weather_code
  )

  const resultado: ClimaInfo = {
    cidade,
...
function obterFeriadosBrasileiros(
  ano?: number
): FeriadoInfo[] {
  const anoAlvo = ano ?? new Date().getFullYear()
  const cacheKey = `feriados:${anoAlvo}`
  const cached = obterDoCache<FeriadoInfo[]>(cacheKey)
  if (cached) return cached

  const FERIADOS: { nome: string; data: string }[] = [
    {
  ...
  const anoFeriado = FERIADOS.filter((f) =>
    f.data.startsWith(String(anoAlvo))
  )
  const resultado: FeriadoInfo[] = []
  ...
  for (const f of anoFeriado) {
    ...
    const dias = Math.round(
      (dataFeriado - hoje.getTime()) / 86400000
    )
...
export async function obterHorario(
  ...
  try {
    const resultado =
      await obterHorarioWorldTimeAPI(timezone)
    if (resultado) registrarSucesso()
    else registrarFalha()
    return resultado
  } catch {
    registrarFalha()
...
export async function obterGeoPorIP(): Promise<GeoInfo | null> {
  try {
...
export async function obterContextoRealtime(
  timezone?: string,
  cidadeClima?: string
): Promise<RealtimeContext> {
  const geo = await obterGeoIPAPI()
  const tz =
    timezone ?? geo?.timezone ?? 'America/Sao_Paulo'
  ...
  const proximo = todosFeriados.find(
    (f) => new Date(f.data) >= hoje
  )

  const calendario: FeriadoInfo | null = proximo
    ? {
        nome: proximo.nome,
        data: proximo.data,
        ...
        calendario: todosFeriados
          .map(
            (f) =>
              `${f.data.slice(5).replace('-', '/')} — ${f.nome}`
          )
          .join('\n'),
      }
    : null

  if (horario) registrarSucesso()
  if (clima) registrarSucesso()
  if (!horario || !clima) registrarFalha()
...
export function formatarContextoRealtime(
  ctx: RealtimeContext
): string {
  const partes: string[] = []
  ...
  if (ctx.geo) {
    partes.push(
      `📍 **Localização:** ${ctx.geo.cidade}, ${ctx.geo.estado}, ${ctx.geo.pais} (IP: ${ctx.geo.ip})${
        ctx.geo.moeda ? ` · Moeda: ${ctx.geo.moeda}` : ''
      }`
    )
  }
  ...
  if (ctx.horario) {
    const h = ctx.horario
    partes.push(
      `🕐 **Horário:** ${h.horaPT} · ${h.dataPT} · ${h.timezone} · ${h.day_of_week} · semana ${h.week_number}${h.dst ? ' (DST)' : ''}`
    )
  }
  ...
  if (ctx.clima) {
    const c = ctx.clima
    partes.push(
      `🌤️ **Clima em ${c.cidade}:** ${c.icone} ${c.condicao} · ${c.temperatura_c}°C (sensação ${c.sensacao_c}°C) · umidade ${c.umidade_pct}% · vento ${c.vento_kmh}km/h${
        c.indice_uv !== undefined
          ? ` · UV ${c.indice_uv}`
          : ''
      }${
  ...
  if (ctx.calendario) {
    partes.push(
      `📅 **Calendário:** ${ctx.calendario.calendario}`
    )
  }
...
function codigoParaCondicao(code: number): {
  condicao: string
  icone: string
} {
  const map: Record<
    number,
    { condicao: string; icone: string }
  > = {
  ...
  return (
    map[code] ?? { condicao: 'Desconhecido', icone: '❓' }
  )
}
