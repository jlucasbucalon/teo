...
function ctxMaximo(ram: RAMInfo, vramGB: number): number {
  if (vramGB >= 12) return 65536
  if (vramGB >= 8)  return 32768
  if (vramGB >= 6)  return 16384
  if (vramGB >= 4)  return 8192

  // CPU-only ou VRAM muito pequena: escala pela RAM livre
  if (ram.livreGB >= 16) return 8192
  if (ram.livreGB >= 8)  return 4096
  if (ram.livreGB >= 4)  return 2048
  return 1024
}
...
function modoLocal(ram: RAMInfo, vramGB: number): ConfigRaciocinio {
...
function modoProfundo(ram: RAMInfo, vramGB: number): ConfigRaciocinio {
...
export function obterRaciocinio(
  modo: ModoRaciocinio,
  hw: { ram: RAMInfo; gpu: GPUInfo }
): ConfigRaciocinio {
...
let modoAtual: ModoRaciocinio = 'local'

export function getModo(): ModoRaciocinio {
  return modoAtual
}

export function setModo(novo: string): boolean {
...
export function numBatch(hw: HardwareInfo): number {
...
export function relatorioRaciocinio(
  r: ConfigRaciocinio
): string {
  return [
    `Modo     : ${r.nome}`,
