...
export interface GPUInfo {
  tipo: GPUTipo
  vramGB: number
  num_gpu: number  // máximo teórico de layers para este GPU
}
...
export interface CapacidadeInfo {
  ...
  cpuPct: number     // 0.0–1.0 fração do total
  ramPct: number
  gpuPct: number
}
...
function detectarRAM(): RAMInfo {
  const totalGB = os.totalmem() / 1024 ** 3
  const livreGB = os.freemem() / 1024 ** 3
  return { totalGB, livreGB }
}

// ── GPU ──────────────────────────────────────────────────────

function detectarGPU(): GPUInfo {
  ...
  // ── AMD ROCm (Linux) ─────────────────────────────────────
  try {
    const out = execSync('rocm-smi --showmeminfo vram --csv', {
      timeout: 3000,
  ...
  try {
    const out = execSync(
      'lspci 2>/dev/null | grep -i "VGA\\|Display\\|3D"',
      { timeout: 3000, stdio: ['pipe', 'pipe', 'ignore'] }
    )
      .toString()
      .toLowerCase()
...
function gpuIntelIntegrada(): GPUInfo {
  const totalGB = os.totalmem() / 1024 ** 3
...
export function detectarHardware(): HardwareInfo {
...
export function relatorioHardware(hw: HardwareInfo): string {
